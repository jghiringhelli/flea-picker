...
<!-- ForgeCraft managed | 2026-03-05 | target: claude -->
...
## Project Identity
- **Repo**: https://github.com/your-org/flea-picker
- **Primary Language**: TypeScript
- **Framework**: React + Vite (static SPA)
- **Domain**: Browser-based casual arcade game
- **Sensitive Data**: None
...
| Term | Definition |
|------|------------|
| `Flea` | The primary target entity. Has position, velocity, jump behaviour, and visibility state. |
| `Dog` | The play-field entity. Rendered as a top-down fur canvas. Acts as the flea habitat. |
| `Tool` | An instrument the player uses (Tweezers or Comb). Each has distinct capture mechanics. |
| `Scenario` | A level configuration: flea count, jump speed, fur density, unlocked tools, time limit. |
| `Catch` | Successful removal of a flea in a single interaction. |
| `Miss` | Failed interaction that triggers a flea scatter event. |
| `Scatter` | A ripple-effect where nearby fleas jump simultaneously on a miss. |
| `GameLoop` | The fixed-timestep update cycle driving all entity movement and state changes. |
| `GameState` | The XState-managed finite-state machine tracking the overall session lifecycle. |

## Architecture Quick Reference
- Entry point: `src/app/main.tsx`
- Game engine: `src/features/game-engine/` — pure TS, zero React imports
- React UI shell: `src/features/game-ui/` — presentation only
- State machine: `src/features/game-state/machine.ts` — XState v5
- Scenario configs: `src/shared/config/scenarios.ts`
- Asset pipeline: `public/assets/` — sprites, audio

## Key Constraints
- Zero server. Static SPA deployed to GitHub Pages / Netlify.
- Canvas rendering via `<canvas>` element in a React ref — no DOM manipulation outside it.
- Game loop must use fixed timestep (60 Hz) with render interpolation.
- All numeric constants (speeds, sizes, timings) MUST live in `src/shared/config/constants.ts`.
